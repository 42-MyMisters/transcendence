...
import { ApiBadRequestResponse, ApiBody, ApiConsumes, ApiForbiddenResponse, ApiInternalServerErrorResponse, ApiOkResponse, ApiOperation, ApiUnauthorizedResponse } from '@nestjs/swagger';
...
export class UserController {
	...
	@ApiOkResponse({ description: "쿠키에 refeshToken 값 리턴, accessToken 비공개로 리턴" })
	@swagger.ApiUnauthorizedResponse({
		description: "Wrong authentication code"
	})
	@Post('/2fa/toggle/confirm')
	@UseGuards(JwtAuthGuard)
	async authConfirm(@Req() request, @Res() res: Response, @Body() twoFactorConfirmDto: TwoFactorConfirmDto) {
	...
	@swagger.ApiNotFoundResponse({description: "uid에 해당하는 아이디는 있으나, 팔로우되어있지 않은경우"})
	@Post('/unfollow/:uid')
	@UseGuards(Jwt2faAuthGuard)
